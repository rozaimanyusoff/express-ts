...
export const createStockRequest = async (data: any) => {
    ...
    try {
        ...
        const [result]: any = await conn.query(
            `INSERT INTO stock.rt_stock_request (
        request_ref_no, requested_by, requested_at, verified_by, verified_at, verification_status,
        processed_by, processed_at, collected_by, collected_at, date_out, department, team_name,
        ...
        if (Array.isArray(items) && items.length > 0) {
            for (const item of items) {
                ...
                await conn.query(
                    `INSERT INTO stock.rt_stock_request_items (stock_out_id, item_id, item_code, item_name, qty, approved_qty, remarks)
           VALUES (?, ?, ?, ?, ?, ?, ?)`,
...
export const getStockRequests = async () => {
    const [rows] = await pool.query('SELECT * FROM stock.rt_stock_request');
    return rows;
};

export const getStockRequestById = async (id: number) => {
    const [rows] = await pool.query('SELECT * FROM stock.rt_stock_request WHERE id = ?', [id]);
    const request = (rows as RowDataPacket[])[0];
    if (!request) return null;
    const [items] = await pool.query('SELECT * FROM stock.rt_stock_request_items WHERE stock_out_id = ?', [id]);
    request.items = items;
    return request;
};
...
export const updateStockRequest = async (id: number, data: any) => {
    ...
    const [result] = await pool.query(
        `UPDATE stock.rt_stock_request SET
      request_ref_no=?, requested_by=?, requested_at=?, verified_by=?, verified_at=?, verification_status=?,
      processed_by=?, processed_at=?, collected_by=?, collected_at=?, date_out=?, department=?, team_name=?,
...
export const deleteStockRequest = async (id: number) => {
    ...
    await pool.query('DELETE FROM stock.rt_stock_request_items WHERE stock_out_id = ?', [id]);
    const [result] = await pool.query('DELETE FROM stock.rt_stock_request WHERE id = ?', [id]);
    return result;
};
...
export const updateStockRequestItem = async (id: number, data: any) => {
    ...
    const [result] = await pool.query(
        `UPDATE stock.rt_stock_request_items SET item_id=?, item_code=?, item_name=?, qty=?, approved_qty=?, remarks=? WHERE id=?`,
...
export const deleteStockRequestItem = async (id: number) => {
    const [result] = await pool.query('DELETE FROM stock.rt_stock_request_items WHERE id = ?', [id]);
    return result;
};
