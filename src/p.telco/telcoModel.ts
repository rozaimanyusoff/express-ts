...
const db25 = 'billings'; //
const tables = {
    accounts: `${db25}.telco_accounts`,
    accountSubs: `${db25}.telco_account_subs`,
    contracts: `${db25}.telco_contracts`,
    deptSubs: `${db25}.telco_department_subs`, // Assuming this is a table for department subscriptions
    oldSubscribers: `${db25}.celcomsub`, // Assuming this is a table for old subscribers
    simCards: `${db25}.telco_simcards`,
    simCardSubs: `${db25}.telco_simcard_subs`,
    subscribers: `${db25}.telco_subscribers`,
    ...
    userSubs: `${db25}.telco_user_subs`, // Assuming this is a table for user subscriptions
    vendors: `${db25}.telco_vendors`,
};
...
export async function createAccount(account: any) {
    ...
    const [dupRows] = await pool2.query<RowDataPacket[]>(`SELECT id FROM ${tables.accounts} WHERE account_master = ? LIMIT 1`, [account_master]);
...
export async function createAccountSub(accountSub: any) {
    ...
    const [dupRows] = await pool2.query<RowDataPacket[]>(`SELECT id FROM ${tables.accountSubs} WHERE sub_no_id = ? AND account_id = ? AND status = 'active' LIMIT 1`, [sub_no_id, account_id]);
    if (dupRows && dupRows.length > 0) {
        return dupRows[0].id;
    }
    const [result] = await pool2.query<ResultSetHeader>(
...
export async function createContract(contract: any) {
    ...
    const [dupRows] = await pool2.query<RowDataPacket[]>(`SELECT id FROM ${tables.contracts} WHERE account_id = ? AND product_type = ? AND contract_start_date = ? LIMIT 1`, [account_id, product_type, contract_start_date]);
    if (dupRows && dupRows.length > 0) {
        return dupRows[0].id;
    }
    const [result] = await pool2.query<ResultSetHeader>(
...
export async function createSimCard(simCard: any) {
    ...
    const [dupRows] = await pool2.query<RowDataPacket[]>(`SELECT id FROM ${tables.simCards} WHERE sim_sn = ? LIMIT 1`, [sim_sn]);
    if (dupRows && dupRows.length > 0) {
        return dupRows[0].id;
    }
    const [result] = await pool2.query<ResultSetHeader>(
...
export async function createSubscriber(subscriber: any) {
    ...
    const [dupRows] = await pool2.query<RowDataPacket[]>(`SELECT id FROM ${tables.subscribers} WHERE sub_no = ? LIMIT 1`, [sub_no]);
    if (dupRows && dupRows.length > 0) {
...
export async function createVendor(vendor: any) {
    ...
    const [dupRows] = await pool2.query<RowDataPacket[]>(`SELECT id FROM ${tables.vendors} WHERE name = ? LIMIT 1`, [name]);
    if (dupRows && dupRows.length > 0) {
        return dupRows[0].id;
    }
    const [result] = await pool2.query<ResultSetHeader>(
...
export async function deleteAccount(id: number) {
    await pool2.query(`DELETE FROM ${tables.accounts} WHERE id = ?`, [id]);
}

export async function deleteSubscriber(id: number) {
    await pool2.query(`DELETE FROM ${tables.subscribers} WHERE id = ?`, [id]);
}
export async function deleteTelcoBilling(id: number) {
    await pool2.query(`DELETE FROM ${tables.telcoBilling} WHERE id = ?`, [id]);
}
...
export async function deleteVendor(id: number) {
    await pool2.query(`DELETE FROM ${tables.vendors} WHERE id = ?`, [id]);
}


export async function getAccountById(accountId: number) {
    const [rows] = await pool2.query<RowDataPacket[]>(`SELECT * FROM ${tables.accounts} WHERE id = ?`, [accountId]);
    return rows[0];
}
// ===================== ACCOUNTS =====================
// CRUD for accounts
export async function getAccounts() {
    const [rows] = await pool2.query<RowDataPacket[]>(`SELECT * FROM ${tables.accounts}`);
    return rows;
}
// ===================== ACCOUNT SUBS JOINS =====================
// CRUD for account_subs (assign subs to accounts)
export async function getAccountSubs() {
    const [rows] = await pool2.query<RowDataPacket[]>(`SELECT * FROM ${tables.accountSubs} WHERE status = 'active' ORDER BY effective_date DESC`);
    return rows;
}
export async function getContractById(contractId: number) {
    const [rows] = await pool2.query<RowDataPacket[]>(`SELECT * FROM ${tables.contracts} WHERE id = ?`, [contractId]);
    return rows[0];
}
// ===================== CONTRACTS =====================
// CRUD for contracts
export async function getContracts() {
    const [rows] = await pool2.query<RowDataPacket[]>(`SELECT * FROM ${tables.contracts}`);
    return rows;
}
...
export async function getDepartmentSubs() {
    ...
    const [rows] = await pool2.query<RowDataPacket[]>(`SELECT * FROM ${tables.deptSubs} ORDER BY id DESC`);
    return rows;
}
// ===================== OLD SUBSCRIBERS =====================
...
export async function getSimCards() {
    const [rows] = await pool2.query<RowDataPacket[]>(`SELECT * FROM ${tables.simCards}`);
    return rows;
}
export async function getSimsBySubscriberId(subscriberId: number) {
    const [rows] = await pool2.query<RowDataPacket[]>(`SELECT * FROM ${tables.simCards} WHERE sub_no_id = ?`, [subscriberId]);
    return rows;
}
...
export async function getSubscriberById(id: number) {
    const [rows] = await pool2.query<RowDataPacket[]>(`SELECT * FROM ${tables.subscribers} WHERE id = ?`, [id]);
    return rows[0];
}
export async function getSubscribers() {
    const [rows] = await pool2.query<RowDataPacket[]>(`SELECT * FROM ${tables.subscribers}`);
    return rows;
}
...
export async function getUserSubs() {
    ...
    const [rows] = await pool2.query<RowDataPacket[]>(`SELECT * FROM ${tables.userSubs}`);
    return rows;
}

export async function getVendorById(vendorId: number) {
    const [rows] = await pool2.query<RowDataPacket[]>(`SELECT * FROM ${tables.vendors} WHERE id = ?`, [vendorId]);
    return rows[0];
}
...
export async function getVendors() {
    const [rows] = await pool2.query<RowDataPacket[]>(`SELECT * FROM ${tables.vendors}`);
    return rows;
}
...
export async function updateSubscriber(id: number, subscriber: any) {
    ...
    const [currentRows] = await pool2.query<RowDataPacket[]>(`SELECT * FROM ${tables.subscribers} WHERE id = ?`, [id]);
    const current = currentRows[0];

    // 2. Fetch latest simcard, department, account, user from *_subs tables
    const [[simSub]] = await pool2.query<RowDataPacket[]>(`SELECT sim_id FROM ${tables.simCardSubs} WHERE sub_no_id = ? ORDER BY effective_date DESC LIMIT 1`, [id]);
    const [[deptSub]] = await pool2.query<RowDataPacket[]>(`SELECT department_id FROM ${tables.deptSubs} WHERE sub_no_id = ? ORDER BY effective_date DESC LIMIT 1`, [id]);
    const [[accSub]] = await pool2.query<RowDataPacket[]>(`SELECT account_id FROM ${tables.accountSubs} WHERE sub_no_id = ? ORDER BY id DESC LIMIT 1`, [id]);
    const [[userSub]] = await pool2.query<RowDataPacket[]>(`SELECT ramco_id FROM ${tables.userSubs} WHERE sub_no_id = ? ORDER BY id DESC LIMIT 1`, [id]);

    // 3. Insert new row if changed
    if (simcard && (!simSub || simSub.sim_id !== simcard)) {
        await pool2.query(`INSERT INTO ${tables.simCardSubs} (sub_no_id, sim_id, effective_date) VALUES (?, ?, NOW())`, [id, simcard]);
    }
    if (department && (!deptSub || deptSub.dept_id !== department)) {
        await pool2.query(`INSERT INTO ${tables.deptSubs} (sub_no_id, department_id, effective_date) VALUES (?, ?, NOW())`, [id, department]);
    }
    if (account && (!accSub || accSub.account_id !== account)) {
        await pool2.query(`INSERT INTO ${tables.accountSubs} (sub_no_id, account_id, effective_date) VALUES (?, ?, NOW())`, [id, account]);
    }
    if (user && (!userSub || userSub.ramco_id !== user)) {
        await pool2.query(`INSERT INTO ${tables.userSubs} (sub_no_id, ramco_id, effective_date) VALUES (?, ?, NOW())`, [id, user]);
    }
