...
function buildPortalUrl(id: number, action: 'recommend' | 'approve', payload: any, ttl: string | number) {
...
export async function sendApprovalEmail(reqId: number) {
  ...
  const payload = { ramco_id: approverRamco || '', contact: to, req_id: reqId };
  ...
  const emailBody = vehicleMaintenanceAuthorizationEmail({
    requestNo: reqId,
    date: formattedDate,
    greetingName: (approver && (approver as any).full_name) || applicant,
    role: 'Approval',
    applicant,
    deptLocation,
    vehicleInfo,
